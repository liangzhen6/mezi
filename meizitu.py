...
def mztu_crawler(max_threads = 1):
	crawl_queue = MogoQueue('meinvxiezhenji', 'crawl_queue') #这个是我们获取URL 的队列
	def pageurl_crawler():
		while True:
			try:
				url = crawl_queue.pop()
...
def process_crawler():
	...
	for i in range(num_cpus):
		p = multiprocessing.Process(target = mztu_crawler) #创建进程
		p.start() #启动进程
